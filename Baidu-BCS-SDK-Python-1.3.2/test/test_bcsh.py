...
ENABLE_LARGE = False
HOST_BUCKET = 'http://bs.baidu.com/pybcstest' # this should match with ~/.bcsh
...
class TestBCSH(unittest.TestCase):
    def __test_singlefile(self, local_file, args):
        ...
        r = system('%s upload %s %s %s ' % (CMD, local_file, remote_path, args))
        print r
        assert r == '', 'return of system is not empty'

        r = system('%s download %s %s %s' % (CMD, remote_path, local_download_file, args))
        assert r == '', 'return of system is not empty'
        ...
        if iswindows(): # windows cat \n \r 会有问题
            return 
        system('%s cat %s > %s' % (CMD, remote_path, local_download_file))
        assert (r == '')
        assert file_equals(local_file, local_download_file), 'file not equals %s vs %s ' % (local_file, local_download_file)
        os.remove(local_download_file)
    ...
    def test_superfile(self):
        ...
        for local_file in cases:
            for split in ['999k', '1M']: #, '10M', '100M']:
                ...
                system('%s  upload  %s %s -d%s' % (CMD, local_file, remote_path, split))
                system('%s  download %s %s ' % (CMD, remote_path, local_download_file))
                assert file_equals(local_file, local_download_file), 'file not equals %s vs %s ' % (local_file, local_download_file)
                os.remove(local_download_file)
    ...
    def test_dir(self):
        ...
        for local_dir, remote_path in dir_cases:
            ...
            system('%s upload -r %s %s' %(CMD, local_dir, remote_path) )

            print 'sleep 25 second, it need time'
            time.sleep(25) 

            print 'download'
            system('%s download -r %s %s ' %(CMD, remote_path, local_download_dir) )
    ...
    def _test_ls(self):#TODO
        #system('./tools/bcsh.py upload /home/ning/soft/Understand-2.6.599-Linux-32bit.tgz http://bcs.duapp.com/pybcstest/abcd/und.tgz -d10')
        pass
