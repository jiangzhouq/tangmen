...
# 其他(create_bucket, del_bucket, set_acl等)请到管理控制台设置
...
def ll__upload_with_superfile(bcs, src, dst, split):
    ''' 用superfile 方式上传 '''
    logging.info('ll__upload_with_superfile(%s, %s, %d)' % (src, dst, split))
...
def ll__upload(bcs, src, dst, split):
    if iswindows():
        dst = dst.decode('gbk').encode('utf8')

    filesize = os.path.getsize(src) 
    if (filesize > split):
        return ll__upload_with_superfile(bcs, src, dst, split)
    else:
        b, o = parse_bcs_location(dst)
        bucket = bcs.bucket(b)
...
@retry(Exception, tries=3)
def ll__download(bcs_object, local_path):
    if iswindows():
        try:
            local_path = local_path.replace('/', os.path.sep).decode('utf8').encode('gbk')
        except:
            pass
    logging.info('get %s -> %s' % (str(bcs_object), local_path) )
    local_dir = os.path.dirname(local_path)
    if not os.path.exists(local_dir):
        logging.info('mkdirs %s' % local_dir )
...
class UploadThread(threading.Thread):
    def __init__(self, bcs, split, upload_lst):
        threading.Thread.__init__(self)
        self.bcs = bcs
        self.split = split
        self.upload_lst = upload_lst

    def run(self):
        for (src, dst) in self.upload_lst:
            ll__upload(self.bcs, src, dst, self.split)
...
class DownloadThread(threading.Thread):
    ...
    def run(self):
        for (obj, local_path) in self.download_lst:
            ll__download(obj, local_path)

def ll__upload_dry(bcs, src, dst):
    print '%s > %s' % (src, dst)
...
def upload(bcs, src, dst, recursive, split):
    ...
    global args
    if len(upload_lst) <= args.thread: #单线程
        logging.info('use 1 thread to upload ' )
        UploadThread(bcs, split, upload_lst).run()
        return
    logging.info('use %d thread(s) upload ' % (args.thread))
    for sub_lst in chunks(upload_lst, len(upload_lst)/ args.thread):
        UploadThread(bcs, split, sub_lst).start()
...
def download(bcs, remote_path, local_path, recursive):
    ...
    if not recursive:
        ...
        bucket = bcs.bucket(b)
        ll__download(bucket.object(o), local_path)
        return
    ...
    def get_local_path(item_remote_path):
        return os.path.join(local_path, item_remote_path[len(remote_path):])

    download_list = [(item , get_local_path(str(item))) for item in lst]


    global args
    if len(download_list) <= args.thread:
        logging.info('use 1 thread to download' )
        DownloadThread(download_list).run()
        return

    logging.info('use %d thread(s) download' % (args.thread))
    for sub_lst in chunks(download_list, len(download_list)/ args.thread):
        DownloadThread(sub_lst).start()

    #for (item,item_local_path) in download_list:
        #try:
            #ll__download(item, item_local_path)
        #except:
...
if __name__ == "__main__":
    ...
    parser_shared.add_argument('-v', '--verbose', action='count',
                        help="verbose") 
    parser_shared.add_argument('-t', '--thread', default='10', # default 10 thread
                               help='thread , only for upload now')
